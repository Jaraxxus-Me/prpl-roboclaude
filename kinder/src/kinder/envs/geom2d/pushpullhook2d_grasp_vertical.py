"""Easiest PushPullHook2D curriculum variant: grasp a vertical hook.

Same scene as PushPullHook2D (table, buttons, hook, robot) with identical
geometry. Two changes:
  1. The hook is always initialized vertically (theta = pi/2).
  2. Success = the robot suctions the hook (instead of pressing buttons).
"""
...
class PushPullHook2DGraspVerticalEnvConfig(
    Geom2DRobotEnvConfig, metaclass=FinalConfigMeta
):
    """Config for PushPullHook2DGraspVerticalEnv().

    Identical to PushPullHook2DEnvConfig except the hook's theta is fixed.
    """
    ...
    # ---- CHANGED: hook theta is fixed to pi/2 (vertical). ----
    # Position bounds are the same x-range but y is fixed since theta
    # is constant, so we sample only x within the original x-range.
    hook_init_pose_bounds: tuple[SE2Pose, SE2Pose] = (
        SE2Pose(world_min_x, table_pose.y - hook_shape[1] / 4, np.pi / 2),
        SE2Pose(
...
class ObjectCentricPushPullHook2DGraspVerticalEnv(
    ObjectCentricGeom2DRobotEnv[PushPullHook2DGraspVerticalEnvConfig]
):
    """Same scene as PushPullHook2D but the hook is always vertical and
    success is grasping (suctioning) the hook."""

    def __init__(
        self,
        config: PushPullHook2DGraspVerticalEnvConfig = (
            PushPullHook2DGraspVerticalEnvConfig()
        ),
        **kwargs,
    ) -> None:
    ...
    def _sample_initial_state(self) -> ObjectCentricState:
        ...
        # Sample hook pose (theta is fixed to pi/2 via config bounds).
    ...
    def step(
        ...
        # (kept from original, but not required for success here).
    ...
    # ---- CHANGED: success = hook is suctioned by robot ----
...
# Main env class
class PushPullHook2DGraspVerticalEnv(ConstantObjectKinDEREnv):
    """Grasp-vertical-hook variant with a constant number of objects."""

    def _create_object_centric_env(
        self, *args, **kwargs
    ) -> ObjectCentricGeom2DRobotEnv:
        return ObjectCentricPushPullHook2DGraspVerticalEnv(*args, **kwargs)
    ...
    def _create_env_markdown_description(self) -> str:
        return (
            ...
            "and a target button. "
            "The hook is always initialized vertically (theta = pi/2). "
            "The goal is to navigate to the hook and grasp it using the "
            "robot's vacuum gripper."
        )
    ...
    def _create_variant_specific_description(self) -> str:
        return (
            "This variant has one hook (always vertical), one movable button, "
    ...
    def _create_reward_markdown_description(self) -> str:
        return (
            ...
            "robot successfully suctions the hook (termination, reward 0.0)."
        )

    def _create_references_markdown_description(self) -> str:
        return (
            "This is an easy curriculum variant of PushPullHook2DEnv, "
            "focusing only on the grasping subtask with a fixed hook orientation."
        )
