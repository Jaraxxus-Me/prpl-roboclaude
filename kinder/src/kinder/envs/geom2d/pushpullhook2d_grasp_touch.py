"""PushPullHook2D curriculum variant: grasp hook and touch movable button.

Same scene as PushPullHook2D (table, buttons, hook, robot) with identical
geometry. Two changes compared to the original:
  1. The hook theta is sampled uniformly in [pi/4, 3*pi/4] (same as original).
  2. Success = the robot suctions the hook AND the movable button has moved
     from its initial position (instead of pressing the target button).
"""
...
class PushPullHook2DGraspTouchEnvConfig(
    Geom2DRobotEnvConfig, metaclass=FinalConfigMeta
):
    """Config for PushPullHook2DGraspTouchEnv().

    Identical to PushPullHook2DEnvConfig except the success condition is
    grasping the hook AND the movable button having moved.
    ...
    # Threshold for detecting that the movable button has moved.
    movable_button_moved_threshold: float = 1e-3
...
class ObjectCentricPushPullHook2DGraspTouchEnv(
    ObjectCentricGeom2DRobotEnv[PushPullHook2DGraspTouchEnvConfig]
):
    """Same scene as PushPullHook2D but success is grasping (suctioning)
    the hook AND having moved the movable button from its initial position.
    Hook orientation is random in [pi/4, 3*pi/4]."""

    def __init__(
        self,
        config: PushPullHook2DGraspTouchEnvConfig = (
            PushPullHook2DGraspTouchEnvConfig()
        ),
        **kwargs,
    ) -> None:
        super().__init__(config, **kwargs)
        # Track the initial position of the movable button.
        self._movable_button_init_x: float = 0.0
        self._movable_button_init_y: float = 0.0
    ...
    def _sample_initial_state(self) -> ObjectCentricState:
        ...
        # Record initial movable button position.
        self._movable_button_init_x = movable_button_pose[0]
        self._movable_button_init_y = movable_button_pose[1]
    ...
    # Success = hook is suctioned by robot AND movable button has moved.
    def _get_reward_and_done(self) -> tuple[float, bool]:
        ...
        # Check if movable button has moved from its initial position.
        button_dx = (
            self._current_state.get(movable_button, "x")
            - self._movable_button_init_x
        )
        button_dy = (
            self._current_state.get(movable_button, "y")
            - self._movable_button_init_y
        )
        button_moved = np.sqrt(button_dx**2 + button_dy**2) > (
            self.config.movable_button_moved_threshold
        )

        if hook_grasped and button_moved:
            return 0.0, True
        return -1.0, False


# Main env class
class PushPullHook2DGraspTouchEnv(ConstantObjectKinDEREnv):
    """Grasp-touch variant with a constant number of objects."""

    def _create_object_centric_env(
        self, *args, **kwargs
    ) -> ObjectCentricGeom2DRobotEnv:
        return ObjectCentricPushPullHook2DGraspTouchEnv(*args, **kwargs)
    ...
    def _create_env_markdown_description(self) -> str:
        return (
            ...
            "and then use it to move the movable button from its initial position."
        )
    ...
    def _create_reward_markdown_description(self) -> str:
        return (
            ...
            "has been displaced from its initial position (termination, "
            "reward 0.0)."
        )
    ...
    def _create_references_markdown_description(self) -> str:
        return (
            ...
            "requiring grasping the hook and using it to touch/move "
            "the movable button."
        )
