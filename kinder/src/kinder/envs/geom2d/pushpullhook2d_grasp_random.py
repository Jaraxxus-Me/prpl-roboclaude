"""PushPullHook2D curriculum variant: grasp a randomly-oriented hook.
...
class PushPullHook2DGraspRandomEnvConfig(
    Geom2DRobotEnvConfig, metaclass=FinalConfigMeta
):
    """Config for PushPullHook2DGraspRandomEnv().

    Identical to PushPullHook2DEnvConfig except the success condition is
    grasping the hook. Hook theta is sampled in [pi/4, 3*pi/4].
    """
...
class ObjectCentricPushPullHook2DGraspRandomEnv(
    ObjectCentricGeom2DRobotEnv[PushPullHook2DGraspRandomEnvConfig]
):
    """Same scene as PushPullHook2D but success is grasping (suctioning)
    the hook. Hook orientation is random in [pi/4, 3*pi/4]."""

    def __init__(
        self,
        config: PushPullHook2DGraspRandomEnvConfig = (
            PushPullHook2DGraspRandomEnvConfig()
        ),
        **kwargs,
    ) -> None:
    ...
    # Success = hook is suctioned by robot.
...
# Main env class
class PushPullHook2DGraspRandomEnv(ConstantObjectKinDEREnv):
    """Grasp-random-hook variant with a constant number of objects."""

    def _create_object_centric_env(
        self, *args, **kwargs
    ) -> ObjectCentricGeom2DRobotEnv:
        return ObjectCentricPushPullHook2DGraspRandomEnv(*args, **kwargs)
    ...
    def _create_references_markdown_description(self) -> str:
        return (
            "This is an intermediate curriculum variant of PushPullHook2DEnv, "
            "focusing only on the grasping subtask with a random hook orientation."
        )
